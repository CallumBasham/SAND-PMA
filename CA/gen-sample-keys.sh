...
# Edited by Callum Basham <callum.basham@warwick.ac.uk>
set -eu
...
openssl req -new -newkey rsa:4096 -days 3650 -nodes -x509 \
    ...
    -subj "/C=UK/ST=Warwickshire/L=Coventry/O=MEME/emailAddress=Callum@meme.cyber.test" \
    -config openssl.cnf

# Create server key and cert
openssl req -new -nodes -config openssl.cnf -extensions server \
    -keyout sample-ca/server.key -out sample-ca/server.csr \
    -subj "/C=UK/ST=Coventry/O=MEME/CN=OpenVPN.meme.cyber.test/emailAddress=OpenVPN@meme.cyber.test"
...
openssl req -new -nodes -config openssl.cnf \
    ...
    -subj "/C=UK/ST=Coventry/O=MEME/CN=R-SysAdm/emailAddress=R-SysAdm@meme.cyber.test"
openssl ca -batch -config openssl.cnf \
    -out sample-ca/client.crt -in sample-ca/client.csr

# Create password protected key file
openssl rsa -aes256 -passout pass:R-SysAdmPass \
    -in sample-ca/client.key -out sample-ca/client-pass.key

# Create pkcs#12 client bundle
openssl pkcs12 -export -nodes -password pass:R-SysAdmPass \
...
openssl req -new -nodes -config openssl.cnf \
    ...
    -subj "/C=UK/ST=Coventry/O=MEME/CN=R-SysAdm-Revoked/emailAddress=R-SysAdm@meme.cyber.test"
openssl ca -batch -config openssl.cnf \
